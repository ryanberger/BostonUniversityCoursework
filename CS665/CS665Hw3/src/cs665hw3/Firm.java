...
public class Firm {
    ...
    public static void main(String[] args) {
		...
		for (String e: employees)
		{
            ...
            {
                emp = new Employee();
                emp.setPayBehavior(new ManagerPay(employeeGroup[0],
                        employeeType,
                        Double.parseDouble(employeeGroup[2])));
                emp.setMonthlyData(new ManagerAllocation(projectShares));
            }
            else if(employeeType.equals("Hourly"))
            {
                emp = new Employee();
                emp.setPayBehavior(new HourlyPay(employeeGroup[0],
                        employeeType, Double.parseDouble(employeeGroup[2])));
                emp.setMonthlyData(new
                        HourlyEmployeeAllocation(projectShares, totalHours));
            }
            else if(employeeType.equals("Exempt"))
            {
                emp = new Employee();
                emp.setPayBehavior(new ExemptPay(employeeGroup[0],
                        employeeType,
                        Double.parseDouble(employeeGroup[2])));
                emp.setMonthlyData(new
